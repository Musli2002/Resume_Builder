...
client = openai.OpenAI(api_key="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")
...
def generate_resume_with_reportlab(data):
    ...
    # Add Hobbies
    if data.get("hobbies"):
        content.append(Paragraph(f"<font color='darkblue'>{render_markdown_text(data['hobbies'])}</font>", normal_style))
        content.append(Spacer(1, 20))
...
hobbies = st.text_input("hobbies", placeholder="Enter your hobbies")
...
if st.button("Generate Resume"):
    resume_data = {
        ...
        "hobbies": generate_refined_resume(
            "create some hobbies for the resume and make it short,precise & to the point as per" + hobbies, job_profile
        )
    }
    print(resume_data)
